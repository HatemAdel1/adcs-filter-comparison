...
%% MODULE 6 — Monte Carlo Statistical Analysis
fprintf('>>> Running: Monte Carlo Statistical Analysis (50 runs)\n');
run('monte_carlo_statistical_analysis.m');
...
fprintf('\nMonte Carlo (N=50) mean RMSE:\n');
fprintf('  EKF : %.4f deg (std = %.4f)\n', MC.rmse.EKF.mean, MC.rmse.EKF.std);
fprintf('  UKF : %.4f deg (std = %.4f)\n', MC.rmse.UKF.mean, MC.rmse.UKF.std);
fprintf('  PF  : %.4f deg (std = %.4f)\n', MC.rmse.PF.mean, MC.rmse.PF.std);
if MC.stats.p_ukf_ekf < 0.05
    fprintf('  UKF vs EKF: p = %.2e (significant ***)\n', MC.stats.p_ukf_ekf);
else
    fprintf('  UKF vs EKF: p = %.2e (n.s.)\n', MC.stats.p_ukf_ekf);
end
fprintf('============================================================\n');
